...
typedef struct{
	...
	char		caption_str[256];			// 現在のキャプション
	char		pre_caption_str[256];		// 直前のキャプション
	char		status_top_name[256];		// 上部ステータスエリア名
	char		status_buttom_name[256];	// 下部ステータスエリア名
	MENU_ITEM	menu[MENU_NUM];				// メニュー[メニュー個数] 現在使用しているメニュー
	MENU_ITEM	pre_menu[MENU_NUM];			// メニュー[メニュー個数] 直前のメニュー 
	int			sem_id;						// セマフォ識別子
